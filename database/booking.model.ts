...
BookingSchema.pre('save', async function () {
    // Use 'this' as any or IBooking to access Mongoose document methods
    ...
    if (booking.isModified('eventId') || booking.isNew) {
      try {
        ...
        if (!eventExists) {
          ...
          // Throwing the error stops the save and returns it to your API
          throw error; 
        }
      } catch (err: any) {
        // If it's our ValidationError, re-throw it
        if (err.name === 'ValidationError') throw err;
  
        // Otherwise, handle database/ID format errors
        const validationError = new Error('Invalid events ID format or database error');
        validationError.name = 'ValidationError';
        throw validationError;
      }
    }
    // When an async hook reaches the end, it is the same as calling next()
  });
